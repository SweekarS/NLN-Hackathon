...
export async function scheduleDailyReminders(enabled: boolean, config: NotificationConfig, tasksLeft: boolean) {
  ...
  const scheduledPayloads: { title: string; body: string; time: { hour: number; minute: number }; skipIfDone: boolean }[] = [];
